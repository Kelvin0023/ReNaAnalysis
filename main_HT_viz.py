...
head_fusion = 'max'
channel_fusion = 'max'  # TODO when plotting the cross window actiavtion
sample_fusion = 'max'  # TODO
...
for f_index, (train, test) in enumerate(skf.split(X, Y)):
    if load_saved_rollout:
        with open(os.path.join(data_root, f'{rollout_file_name}_{f_index}.pkl'), 'rb') as f:
            rolls = pickle.load(f)
        with open(os.path.join(data_root, 'labels.pkl'), 'rb') as f:
        ...
        with open(os.path.join(data_root, f'{rollout_file_name}_{f_index}.pkl'), 'wb') as f:
            pickle.dump(rolls, f)
        with open(os.path.join(data_root, 'labels.pkl'), 'wb') as f:
    ...
    for roll_depth in range(model.depth):
        ...
        cross_window_activates = np.mean(this_roll, axis=1)  # finding max activates across channels
        ...
        plt.plot(list(range(1, model.num_windows + 1)), np.max(cross_window_activates, axis=0), label="Max across samples")

        plt.xticks(ticks=x_ticks, labels=x_labels)
        plt.xlabel("100 ms windowed bins")
        plt.ylabel("Cross-window attention activation")
        plt.title(f'Cross-window attention: {f_index}-fold, HT depth {roll_depth} of {model.depth}')
        plt.legend()
        plt.tight_layout()
        plt.show()
        ...
        for class_index, e_name in enumerate(event_names):
            ...
            for window_i in range(model.num_windows):
                activation = this_roll[labels == class_index][:, :, window_i]
                activation = np.mean(activation, axis=0)
                plot_topomap(activation, info, axes=axes[window_i - 1], show=False, res=512, vlim=(np.min(this_roll), np.max(this_roll)))
        ...
        fig.suptitle(f"Attention to CLS token: {f_index}-fold, HT depth {roll_depth} of {model.depth}", fontsize='x-large')
        plt.show()