...
if __name__ == "__main__":
    ...
    locking_filters = {
                       ...
                       'VS-Patch-Sim': [lambda x: type(x) == Fixation and x.is_first_long_gaze  and x.block_condition == conditions['VS'] and x.detection_alg == 'Patch-Sim' and x.dtn == dtnn_types["Distractor"],
                        ...
                        'RSVP-Item-Onset': [lambda x: x.block_condition == conditions['RSVP'] and x.dtn_onffset and x.dtn==dtnn_types["Distractor"] and x.block_id < 3,
                                            lambda x: x.block_condition == conditions['RSVP'] and x.dtn_onffset and x.dtn == dtnn_types["Target"] and x.block_id < 3],
    ...
    end_locking = time.time()
    print(f'locking={end_locking - start_locking}')
    start_filter = time.time()
    event_filters = locking_filters[selected_locking]
    end_filter = time.time()
    print(f'filter={end_filter - start_filter}')
    ...
    end_epoch = time.time()
    print(f'epoch={end_epoch - start_epoch}')
    start_rsquare = time.time()
    r_square_test(rdf, event_names, event_filters, title=f'{selected_locking}')
    end_rsquare = time.time()
    print(f'rsquare={end_rsquare - start_rsquare}')

    if is_regenerate_epochs:
        x, y, _, _ = epochs_to_class_samples(rdf, event_names, event_filters, data_type='both', rebalance=True)
    ...
    start_reducedim = time.time()
    x_eeg = np.copy(x[0])
    if is_reduce_eeg_dim:
        x[0] = compute_pca_ica(x[0], num_top_compoenents)
    end_reducedim = time.time()
    print(f'reducedim={end_reducedim - start_reducedim}')
    ...
    end_hdca = time.time()
    print(f'hdca={end_hdca - start_hdca}')
    end = time.time()
    print(f'overall={end - start_time}')
