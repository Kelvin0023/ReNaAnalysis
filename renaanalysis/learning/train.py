...
def eval_model(x_eeg, x_pupil, y, event_names, model_name, eeg_montage,
    ...
    model_performance, training_histories = _run_model(model_name, x_eeg_znormed, x_eeg_pca_ica, x_pupil_znormed, y, event_names, test_name, ht_output_mode, eeg_montage, ht_lr=ht_lr, ht_l2=ht_l2, n_folds=n_folds, exg_resample_rate=exg_resample_rate, pca=pca, ica=ica, viz_rebalance=viz_rebalance)
...
def _run_model(model_name, x_eeg, x_eeg_pca_ica, x_pupil, y, event_names, test_name,
               ...
               n_folds=10, exg_resample_rate=200, viz_rebalance=False):
    """
...
def grid_search_ht(grid_search_params, x_eeg, x_eeg_pca_ica, y, event_names, n_folds, test_name='', exg_resample_rate=128, is_plot_conf=False, is_pca_ica=True, is_by_channel=False, reload_saved_samples=True, participant=None, session=None, regenerate_epochs=True, viz_rebalance=False, model_name='HT-sesup'):
    ...
    models_param = {}
    training_mode = 'self-sup pretrain' if model_name == 'HT-sesup' else 'classification'
    for params in param_grid:
        print(f"Grid search params: {params}. Searching {len(total_training_histories) + 1} of {len(param_grid)}")
        model = HierarchicalTransformer(num_timesteps, num_channels, exg_resample_rate, num_classes=2, extraction_layers=None,
    ...
    for f_index, (train, val) in enumerate(skf.split(X[0] if isinstance(X, list) else X, Y)):
        ...
        if training_mode == 'classification':
            ...
            #     prev_para.append(param.cpu().detach().numpy())
                train_auc, train_loss, train_accuracy, num_train_standard_error, num_train_target_error, train_y_all, train_y_all_pred = _run_one_epoch_classification(model_copy, train_dataloader, criterion, last_activation, optimizer, mode='train', device=device, l2_weight=l2_weight, test_name=test_name, verbose=verbose)
                ...
                val_auc, val_loss, val_accuracy, num_val_standard_error, num_val_target_error, val_y_all, val_y_all_pred = _run_one_epoch_classification(model_copy, val_dataloader, criterion, last_activation, optimizer, mode='val', device=device, l2_weight=l2_weight, test_name=test_name, verbose=verbose)
                if is_plot_conf_matrix:
...
def _run_one_epoch_classification(model, dataloader, criterion, last_activation, optimizer, mode, l2_weight=1e-5, device=None, test_name='', verbose=1, check_param=1):
    """
...
def _run_one_epoch_self_sup(model, dataloader, criterion, optimizer, mode, l2_weight=1e-5, device=None, test_name='', verbose=1, check_param=1):
    """
