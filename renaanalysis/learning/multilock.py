...
def eval_multi_locking_model(rdf, event_names, regenerate_epochs=True, exg_resample_rate=200):
    ...
    for (p, s), block_ids in ps_vs_block_ids.items():
        for block_i, block_id in enumerate(block_ids):
            ...
            item_samples, y, item_epochs, event_ids = rena_epochs_to_class_samples_rdf(rdf, [str(x) for x in item_ids_in_this_block], event_filters, participant=p, session=s, data_type='both', reject=None)
