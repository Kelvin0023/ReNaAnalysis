...
def build_train_rnn(x_train, x_test, y_train, y_test, epochs=250, batch_size=64, use_GPU=True):
    clear_session()
...
def plot_cm_sinple_results(train_histories, note=''):
    acc_matrix = np.array([entry[1][1] for scn, entry in train_histories]).reshape((3, 3))
    im = ax.imshow(acc_matrix,vmin=0, vmax=1)

    text = plt.text(j, i, round(acc_matrix[i, j], 3),
                   ha="center", va="center", color="black", size=20)
...
def plot_roc_multiclass(n_classes, y_score, y_test, classes, ax=None, zoom=False):
    # Plot linewidth.
    lw = 2
